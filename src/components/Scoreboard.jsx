...
export default function Scoreboard({ state, big = false }) {
  ...
  const progress = state.duration
    ? ((state.remaining ?? state.duration) / state.duration)
    : 1;
  ...
  return (
    <div className={big ? 'scoreboard big' : 'scoreboard'}>
      ...
      <div className={`score-team blue ${blueWins ? 'leading' : ''}`}>
        <span>{state.blueTeam}</span>
        <strong>{state.blueScore}</strong>
      </div>
      ...
      <div className="timebox">
        ...
        <em className={`status-badge ${String(state.displayStatus).toLowerCase()}`}>
          {state.displayStatus}
        </em>
      </div>
    </div>
